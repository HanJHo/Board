...
import com.example.boardproj.service.FileService;
...
public class BoardController {
    ...
    //등록보기페이지
    @GetMapping("/register")
    public String registerGet(BoardDTO boardDTO, PageRequestDTO pageRequestDTO){
...
//        log.info("컨트롤러로 들어온값 : " + multipartFile.getOriginalFilename());
        ...
        if (bindingResult.hasErrors()) {
            log.info("유효성 검사가 문제가 있다 아래 로그는 모든 문제를 출력해준다.");
...
//            }
    ...
    public String del(Long bno){
        ...
        boardImgService.removeWithBoard(bno);
        replyService.removeWithBoard(bno);
